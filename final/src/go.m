...
for i = 1:length(algorithm_options)
    fprintf('Testing algorithm (%d/%d): %s \n', ...
        i, length(algorithm_options), algorithm_options(i).description);
    
    % snp positions are not used in any algorithm (the 0 argument)
    [current_accuracy, current_accuracy_vector] = cross_validate(...
        algorithm_options(i), train, extracted_train, 0, missing, 6);
    
    accuracy_matrix(:,i) = current_accuracy_vector;
    
    fprintf('Done testing algorithm (%d/%d): %s. Accuracy: %f \n', ...
        i, length(algorithm_options), algorithm_options(i).description, current_accuracy);
end
...
algorithm_options = [...
    ...
    svm_algorithm('-t 2 -g 0.012 -c 1000 -q',7), ...
    entangled_snp_algorithm(20), ...
    nearest_neighbor_algorithm(3,3, 'euclidean'),...
    nearest_neighbor_algorithm(5,5, 'correlation'),...
    svm_algorithm2('-t 2 -g 0.01 -c 100 -q', 30, 10),...
    tree_algorithm(8,0), ...
    adaboost_algorithm(10,40), ...
    histogram_algorithm('-t 1 -d 2 -c 1 -q',4,1)];
