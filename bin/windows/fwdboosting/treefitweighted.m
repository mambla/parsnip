function Tree=treefitweighted(X,yw,varargin)
...
[critval,cutval]=Ccritval(xxxx,Ccum,rows,xcat,pratio,Pt,impurity,critfun,bestcrit);
         else
            ycum = cumsum(Cnode(idx,:) - ybar);  % centered response cum. sum
            [critval,cutval]=Rcritval(xxxx,ycum,rows,xcat);
         end
...
%v=1-sum(p.^2,2);
v = 1-sum((w.*p).^2,2)./sum((w.*w).^2,2);

%----------------------------------------------------
function v=twoing(Pleft, P1, Pright, P2,w)
%TWOING Twoing index

%v = 0.25 * Pleft .* Pright .* sum(abs(P1-P2),2).^2;
v = 0.25 * Pleft .* Pright .* sum(w.*abs(P1-P2),2).^2;

%----------------------------------------------------
function v=deviance(p,w)
%DEVIANCE Deviance

%v = -2 * sum(p .* log(max(p,eps(class(p)))), 2);
v = -2 * sum(w.* p .* log(max(p,eps(class(p)))), 2);

%----------------------------------------------------
function [critval,cutval]=Ccritval(xxxx,Ccum,rows,iscat,pratio,Pt,impurity,critfun,bestcrit)
...
www = w*ones(1,nclasses);
www = www(rows,:);

% Evaluate criterion as impurity or otherwise
if impurity
   crit = - Pleft.*feval(critfun,P1,www);
   t = (crit>bestcrit);   % compute 2nd term only if it would make a difference
   if any(t)
      crit(t) = crit(t) - Pright(t).*feval(critfun,P2(t,:),www(t,:));
   end
else
   crit = feval(critfun, Pleft, P1, Pright, P2,www);
end
...
%----------------------------------------------------
function [critval,cutval]=Rcritval(xxxx,Ycum,rows,iscat)
